package trabalho2.model.automato;
...
public class Transitions {
    ...
    /**
     * Construtor que tenta copiar todas as transições da outra classe
     * Transitions passada.
     *
    ...
    public Transitions(Transitions t) {
        this.transitions = new HashMap<>();
        HashMap<State, HashMap<Character, ArrayList<State>>> tmp = t.getTransitions();
    ...
    public void addTransitions(Transitions t) {
        ...
        for (State s : tmp.keySet()) {
            HashMap<Character, ArrayList<State>> temp = new HashMap<Character, ArrayList<State>>();
            for (char c : tmp.get(s).keySet()) {
                temp.put(c, new ArrayList<>(tmp.get(s).get(c)));
            }
